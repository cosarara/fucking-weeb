...
; close your eyes for a momment
;(define dnd-target-entry
;  (allocate (foreign-code "sizeof(GtkTargetEntry)")))
;(foreign-code
;  (format #f "GtkTargetEntry* _my_target_entry = ~A;" #$dnd-target-entry))
(foreign-declare
  "static GtkTargetEntry _my_target_table[] = {{ \"text/plain\", 0, 0 }};")

; because the one generated by bind doesn't read the uchar*
; as a string
(define my_gtk_selection_data_get_text
  (foreign-lambda unsigned-c-string
...
(define (build-view-screen window id)
  ...
  (gtk_drag_dest_set
    ...
    (foreign-value "_my_target_table" c-pointer)
    1 GDK_ACTION_COPY)
